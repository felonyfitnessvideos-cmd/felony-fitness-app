...
ADD COLUMN IF NOT EXISTS cycle_session_id UUID REFERENCES cycle_sessions(id) ON DELETE
SET NULL;
-- Add index for performance when querying logs by session
CREATE INDEX IF NOT EXISTS idx_workout_logs_cycle_session ON workout_logs(cycle_session_id)
