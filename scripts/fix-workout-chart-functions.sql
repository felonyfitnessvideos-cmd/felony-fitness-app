-- Fix workout calculation functions for production database
-- These functions are needed for chart data to display properly

-- Drop and recreate the functions with proper error handling
DROP FUNCTION IF EXISTS public.calculate_exercise_1rm(uuid, uuid);
DROP FUNCTION IF EXISTS public.calculate_exercise_weight_volume(uuid, uuid);
DROP FUNCTION IF EXISTS public.calculate_exercise_set_volume(uuid, uuid);
