...
-- Run this in Supabase SQL Editor to fix the Double Baconator logging issue
...
BEGIN
  ...
  IF p_external_food IS NOT NULL THEN
    -- Validate nutritional data using existing function if available
    BEGIN
    ...
    EXCEPTION
      WHEN undefined_function THEN
        -- Function doesn't exist, skip validation
        NULL;
    END;

    -- Suggest/validate category using existing function if available
    BEGIN
      v_category_suggestion := suggest_food_category(p_external_food->>'name');
    EXCEPTION
      WHEN undefined_function THEN
        -- Function doesn't exist, use simple category logic
        v_category_suggestion := CASE 
          WHEN lower(p_external_food->>'name') LIKE '%burger%' OR lower(p_external_food->>'name') LIKE '%baconator%' THEN 'Fast Food'
          WHEN lower(p_external_food->>'name') LIKE '%fries%' THEN 'Sides & Snacks'
          ELSE COALESCE(p_external_food->>'category', 'Miscellaneous')
        END;
    END;
    ...
    IF EXISTS (
      SELECT 1 FROM foods 
      WHERE lower(name) = lower(p_external_food->>'name')
      LIMIT 1
    ) THEN
