...
const WeeklyMealPlannerPage = () => {
  ...
  const loadPlanEntries = async () => {
    ...
    try {
      ...
      // Add nutrition calculation to entries
      const entriesWithNutrition = data.map(entry => ({
        ...entry,
        nutrition: calculateMealNutrition(entry.meals.meal_foods, entry.servings)
      }));
      
      setPlanEntries(entriesWithNutrition);
    } catch (error) {
  ...
  const calculateWeeklyNutrition = () => {
    ...
    currentWeek.forEach((date, dayIndex) => {
      ...
      const dayEntries = planEntries.filter(entry => entry.plan_date === dateStr);
      
      const dayTotals = dayEntries.reduce((acc, entry) => {
        const nutrition = calculateMealNutrition(entry.meals.meal_foods, entry.servings);
        return {
      ...
      weeklyTotals[dateStr] = dayTotals;
    });
